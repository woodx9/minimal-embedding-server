...
app = FastAPI(title="MSE - Minimal Embedding Server", version="0.1.0")
...
def start_server():
    ...
    parser = argparse.ArgumentParser(
        description="MSE - Minimal Embedding Server",
...
示例:
  ...
  mes --host 0.0.0.0 --port 8000
    ...
    parser.add_argument(
        "--host",
        type=str,
        default=os.getenv("MSE_HOST", "0.0.0.0"),
        help="服务器监听地址 (默认: 0.0.0.0，可通过 MSE_HOST 环境变量设置)"
    )
    
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.getenv("MSE_PORT", "8000")),
        help="服务器监听端口 (默认: 8000，可通过 MSE_PORT 环境变量设置)"
    )
    ...
    # 打印启动信息
    print("=" * 60)
    print("MSE - Minimal Embedding Server")
    print("=" * 60)
